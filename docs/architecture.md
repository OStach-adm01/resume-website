...
No selection defaults to Yes. Selecting No downloads without calling the API. Selecting Yes requires a company name; only a successful API response triggers the download. API Gateway validates all request content types through the `$default` model, including `recruiter: true`. Lambda rejects other methods/types, unexpected fields, invalid identifiers, invalid names, and oversized bodies.
